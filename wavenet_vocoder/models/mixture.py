...
def log_sum_exp(x):
	...
	m = tf.reduce_max(input_tensor=x, axis=axis)
	m2 = tf.reduce_max(input_tensor=x, axis=axis, keepdims=True)
	return m + tf.math.log(tf.reduce_sum(input_tensor=tf.exp(x-m2), axis=axis))
...
def discretized_mix_logistic_loss(y_hat, y, num_classes=256,
	...
	assert tf.shape(input=y_hat)[1] % 3 == 0

	nr_mix = tf.shape(input=y_hat)[1] // 3
	...
	log_probs = tf.reduce_sum(input_tensor=log_probs, axis=-1, keepdims=True) + tf.nn.log_softmax(logit_probs, -1)

	if reduce:
		return -tf.reduce_sum(input_tensor=log_sum_exp(log_probs))
	else:
...
def sample_from_discretized_mix_logistic(y, log_scale_min=-7.):
	...
	'''
	assert tf.shape(input=y)[1] % 3 == 0
	nr_mix = tf.shape(input=y)[1] // 3
	...
	temp = tf.random.uniform(logit_probs.get_shape(), minval=1e-5, maxval=1. - 1e-5)
	temp = logit_probs - tf.math.log(-tf.math.log(temp))
	argmax = tf.argmax(input=temp, axis=-1)

	#[batch_size, time_length] -> [batch_size, time_length, nr_mix]
	one_hot = tf.one_hot(argmax, depth=nr_mix, dtype=tf.float32)
	#select logistic parameters
	means = tf.reduce_sum(input_tensor=y[:, :, nr_mix:2 * nr_mix] * one_hot, axis=-1)
	log_scales = tf.maximum(tf.reduce_sum(
		input_tensor=y[:, :, 2 * nr_mix:3 * nr_mix] * one_hot, axis=-1), log_scale_min)

	#sample from logistic & clip to interval
	#we don't actually round to the nearest 8-bit value when sampling
	u = tf.random.uniform(means.get_shape(), minval=1e-5, maximum=1. - 1e-5)
